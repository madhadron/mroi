...
public class Update<T> extends MutatingCommand<T> {
	T r;
	public Update(T r) {
		this.r = r;
	}
	...
	public MZipper<T> operation(MZipper<T> mz) {
		if (mz instanceof JZipper) {
			return new JZipper<T>(((JZipper<T>)mz).lefts, r, ((JZipper<T>)mz).rights);
		} else {
			return new JZipper<T>(((NZipper<T>)mz).lefts, r, ((NZipper<T>)mz).rights);
		}
	}
